...
class WorktreeManager:
    ...
    def run(self, name: str, command: str) -> str:
        ...
        output = run_bash(command, cwd=path, timeout=300)
        self.events.emit("worktree.run.after", task_id=wt.get("task_id"), wt_name=name)
        return output

    def keep(self, name: str, reason: str = "", complete_task: bool = False) -> str:
        wt = self._find(name)
        if not wt:
            return f"Error: Unknown worktree '{name}'"
        ...
        return json.dumps(self._find(name), indent=2, ensure_ascii=False)
    ...
    def closeout(self, name: str, action: str, reason: str = "", force: bool = False, complete_task: bool = False) -> str:
        if action == "keep":
            return self.keep(name, reason, complete_task)
        if action == "remove":
            return self.remove(name, force, complete_task, reason)
